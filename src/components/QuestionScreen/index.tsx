...
const QuestionScreen: FC = () => {
  ...
  const [modalType, setModalType] = useState<'timer' | 'result' | null>(null)
  ...
  useTimer(timer, quizDetails, setEndTime, setTimer, () => setModalType('timer'), !!modalType)

  return (
    <PageCenter>
      <div className="text-app-logo mb-4 text-center md:mb-6">
        <AppLogo className="invert dark:invert-0 mx-auto h-10 w-auto md:h-16" />
      </div>
      <div className="bg-card-bg shadow-card relative mb-4 w-full rounded-xl p-4 pb-20 md:w-[900px] md:px-14 md:pt-8">
        <QuizHeader
      ...
      {modalType && (
        <ModalWrapper
          title={modalType === 'result' ? 'Done!' : 'Your time is up!'}
          subtitle={`You have attempted ${result.length} questions in total.`}
          onClick={handleModal}
          icon={modalType === 'result' ? <CheckIcon /> : <TimerIcon />}
